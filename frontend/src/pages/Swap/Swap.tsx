...
import { batchFetchPoolData, callWithRetry } from "../../utils/batchFetch";
import { checkAndClearPoolsRefetch, triggerPoolsRefetch, logTransaction } from "../../utils/cache";
...
import {
  ...
  getTransferFee,
  getTransferInverseFee
} from "../../utils/swapmath/transferFees";
...
export default function Swap() {
  ...
  useEffect(() => {
    if (wallet.publicKey && checkAndClearPoolsRefetch(wallet.publicKey.toBase58())) {
      setDisplayPools([]);
    }
    let cancelled = false;
    ...
    const loadBatches = async () => {
      ...
      for (let i = 0; i < poolsData.length; i += batchSize) {
        ...
        const enrichedBatch = await batchFetchPoolData(connection, pgmId, batch);
        if (cancelled) break;
  ...
  }, [poolsData, connection, program]);

  const allPools = useMemo<PoolData[]>(() => {
    const source = displayPools.length > 0 ? displayPools : poolsData;
    return source.map((p) => ({
      poolPda: p.poolPda,
      token0: p.tokenMint0,
      token1: p.tokenMint1,
  ...
  const [poolHoverInfo, setPoolHoverInfo] = useState<{
    poolId?: string | null;
    token0?: string | null;
    token1?: string | null;
  } | null>(null);
  ...
  const hidePoolHover = () => {
    clearPoolHoverTimeout();
    poolHoverTimeout.current = window.setTimeout(
      () => setPoolHoverInfo(null),
      150,
    );
  };
  ...
  const activePool = useMemo(() => {
    ...
    else if (routeMatchedPool)
      basePool = { ...poolFromRoute, ...routeMatchedPool };
  ...
  const token0MintParam = useMemo(() => {
    const mint0 =
      (activePool as any)?.token0 ??
      (activePool as any)?.tokenMint0 ??
      (activePool as any)?.token0Mint;
  ...
  const token1MintParam = useMemo(() => {
    const mint1 =
      (activePool as any)?.token1 ??
      (activePool as any)?.tokenMint1 ??
      (activePool as any)?.token1Mint;
  ...
  const [slippage, setSlippage] = useState<number>(() => {
    try {
      const stored = localStorage.getItem('swap_slippage');
  ...
  const [priceDetails, setPriceDetails] = useState<{
    token0ToToken1: string;
    token1ToToken0: string;
  } | null>(null);
  ...
  const parseHumanAmountToBaseUnits = (value: string, decimals: number) => {
    ...
    const fractionPart = (fractionRaw + "0".repeat(decimals)).slice(
      0,
      decimals,
    );
  ...
  const formatBaseUnitsToHuman = (
    value: typeof BN.prototype,
    decimals: number,
  ) => {
  ...
  const isAlreadyProcessedError = (err: any) => {
    const rawMsg = [err?.message, err?.transactionMessage, String(err || "")]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
  ...
  const [searchQuery, setSearchQuery] = useState(() => {
    try {
      return sessionStorage.getItem('swap_searchQuery') || '';
    } catch {
      return '';
    }
  });
  ...
  useEffect(() => {
    ...
    const fetchSpecificAmmConfig = async () => {
      try {
        ...
        if (program) {
          try {
            configAcct = await callWithRetry(() => (program.account as any).ammConfig.fetch(
              configPubkey,
            )) as any;
          } catch (e) {
            console.warn(
              "Failed to fetch via program, trying connection getAccountInfo:",
              e,
            );
          }
        }
        ...
        if (!configAcct) {
          const info = await callWithRetry(() => connection.getAccountInfo(configPubkey));
          if (info) {
        ...
        if (configAcct) {
          const feeRate =
            configAcct.tradeFeeRate ?? configAcct.trade_fee_rate ?? 0;
          const feeRateNum =
    ...
    quoteTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (currentRequestId === quoteRequestIdRef.current) {
          setAmountOut(
            formatBaseUnitsToHuman(quote.receiveAmount, quote.outputDecimals),
          );
  ...
  const updateOutputAmount = async (value: string) => {
    if (value === amountOut && lastEditedField === "output") return; // Only trigger if value or active field actually changes
  ...
  useEffect(() => {
    ...
    const fetchBalances = async () => {
      try {
        ...
        if (dec0 === 0 && !isSol0) {
          try {
            const mInfo = await callWithRetry(() =>
              getMint(
                connection,
                new PublicKey(mint0Str),
                "confirmed",
                TOKEN_PROGRAM_ID,
              ),
            );
        ...
        if (wallet.publicKey) {
          ...
          let tokenProgram0 = TOKEN_PROGRAM_ID;
          let tokenProgram1 = TOKEN_PROGRAM_ID;
          try {
          ...
          if (isSol0) {
            ...
            const ata0 = getAssociatedTokenAddressSync(
              mint0,
              owner,
              true,
              tokenProgram0,
            ...
            if (b0) {
              bal0 =
                b0.value.uiAmount != null
                  ? b0.value.uiAmount
                  : Number(b0.value.amount) / Math.pow(10, dec0);
            }
          }
          ...
          if (isSol1) {
            ...
            if (b1) {
              bal1 =
                b1.value.uiAmount != null
                  ? b1.value.uiAmount
                  : Number(b1.value.amount) / Math.pow(10, dec1);
            }
          }
        }
        ...
        if (mounted) {
          setUserBalances(
            wallet.publicKey ? { token0: bal0, token1: bal1 } : null,
          );
        }
      } catch (e) {
  ...
  const toggleSwapDirection = () => {
    setSwapDirection((current) =>
      current === "token0-to-token1" ? "token1-to-token0" : "token0-to-token1",
    );
  ...
  const loadSwapContext = async (ownerPublicKey?: PublicKey) => {
    const t0 = token0MintParam as PublicKey | undefined;
    const t1 = token1MintParam as PublicKey | undefined;
    if (!t0 || !t1) {
      throw new Error("Selected pool is missing token mint addresses");
    }

    const poolAddr = poolPdaParam;
    if (!poolAddr) {
    ...
    if (program) {
      poolStateAcct = await callWithRetry(() => (program.account as any).poolState.fetch(poolAddr)) as any;
    } else {
      const info = await callWithRetry(() => connection.getAccountInfo(poolAddr));
    ...
    const [observationState] = getObservationAddress(
      poolAddr,
      program
    ...
    const token0Vault =
      poolStateAcct.tokenVault0 || poolStateAcct.token_vault_0;
    const token1Vault =
      poolStateAcct.tokenVault1 || poolStateAcct.token_vault_1;

    const feeConfig0 = await getTransferFeeConfig(connection, t0!);
    const feeConfig1 = await getTransferFeeConfig(connection, t1!);
  ...
  const getDirectionalContext = async (
    ...
    const inputDecimals = Number(
      inputIsToken0 ? (ctx.mint0.decimals ?? 0) : (ctx.mint1.decimals ?? 0),
    );
    const outputDecimals = Number(
      inputIsToken0 ? (ctx.mint1.decimals ?? 0) : (ctx.mint0.decimals ?? 0),
    );
    ...
    const inputTokenProgram = inputIsToken0 ? ctx.inputTokenProgram : ctx.outputTokenProgram;
    ...
    const outputFeeConfig = inputIsToken0 ? ctx.feeConfig1 : ctx.feeConfig0;
  ...
  const fetchTickArraysForQuote = async (ctx: any, resolved: any) => {
    ...
    const startTickIndex =
      Math.floor(currentTick / tickArraySizeTimesSpacing) *
      tickArraySizeTimesSpacing;
    const pgmId = (program as any).programId;

    const [tickArrayCurrent] = getTickArrayAddress(
      ctx.poolAddr,
      startTickIndex,
      pgmId,
    );
    ...
    if (resolved.inputIsToken0) {
      const [taLeft] = getTickArrayAddress(
        ctx.poolAddr,
        startTickIndex - tickArraySizeTimesSpacing,
        pgmId,
      );
      const [taLeft2] = getTickArrayAddress(
        ctx.poolAddr,
        startTickIndex - 2 * tickArraySizeTimesSpacing,
        pgmId,
      );
      orderedAddresses = [tickArrayCurrent, taLeft, taLeft2];
    } else {
      const [taRight] = getTickArrayAddress(
        ctx.poolAddr,
        startTickIndex + tickArraySizeTimesSpacing,
        pgmId,
      );
      const [taRight2] = getTickArrayAddress(
        ctx.poolAddr,
        startTickIndex + 2 * tickArraySizeTimesSpacing,
        pgmId,
      );
    ...
    const arrays = await Promise.all(
      orderedAddresses.map((ta) =>
        callWithRetry(() => (program.account as any).tickArrayState.fetchNullable(ta) as Promise<any>),
      ),
    );
    return arrays.filter(Boolean);
  };

  const quoteExactIn = async (
    humanAmount: string,
    direction: SwapDirection = swapDirection,
  ) => {
    const ctx = await loadSwapContext();
    const resolved = await getDirectionalContext(ctx, undefined, direction);
    const inputBase = parseHumanAmountToBaseUnits(
      humanAmount,
      resolved.inputDecimals,
    );

    let transferFeeIn = new BN(0);
    if (resolved.inputFeeConfig) {
      transferFeeIn = getTransferFee(
        inputBase,
        resolved.inputFeeConfig.feeBasisPoints,
        resolved.inputFeeConfig.maxFee,
      );
    }
    const amountCalculateSpecified = inputBase.sub(transferFeeIn);

    const tickArrays = await fetchTickArraysForQuote(ctx, resolved);

    const activeFeeTierRaw = parseFloat(activePoolFeeTier);
    const feeRateNum = isNaN(activeFeeTierRaw) ? 0 : activeFeeTierRaw * 10000; // e.g. 0.25% -> 2500
    ...
    try {
      if (tickArrays.length > 0) {
        const sim = swapInternal({
          ...
          amountSpecified: amountCalculateSpecified,
          sqrtPriceLimitX64: new BN(0),
          zeroForOne: resolved.inputIsToken0,
          isBaseInput: true,
          feeRate: feeRateNum,
        });
        const receiveAmountRaw = sim.amountCalculated;
        let transferFeeOut = new BN(0);
        if (resolved.outputFeeConfig) {
          transferFeeOut = getTransferFee(
            receiveAmountRaw,
            resolved.outputFeeConfig.feeBasisPoints,
            resolved.outputFeeConfig.maxFee,
          );
        }
        receiveAmount = receiveAmountRaw.sub(transferFeeOut);
  ...
  const quoteExactOut = async (
    humanAmount: string,
    direction: SwapDirection = swapDirection,
  ) => {
    const ctx = await loadSwapContext();
    const resolved = await getDirectionalContext(ctx, undefined, direction);
    const desiredOutputBase = parseHumanAmountToBaseUnits(
      humanAmount,
      resolved.outputDecimals,
    );

    let transferFeeOut = new BN(0);
    if (resolved.outputFeeConfig) {
      transferFeeOut = getTransferInverseFee(
        desiredOutputBase,
        resolved.outputFeeConfig.feeBasisPoints,
        resolved.outputFeeConfig.maxFee,
      );
    }
    const amountCalculateSpecified = desiredOutputBase.add(transferFeeOut);
    ...
    try {
      if (tickArrays.length > 0) {
        ...
        const maxInputRaw = sim.amountCalculated;
        let transferFeeIn = new BN(0);
        if (resolved.inputFeeConfig) {
          transferFeeIn = getTransferInverseFee(
            maxInputRaw,
            resolved.inputFeeConfig.feeBasisPoints,
            resolved.inputFeeConfig.maxFee,
          );
        }
        maxInputPreFee = maxInputRaw.add(transferFeeIn);
        insufficientLiquidity = !sim.allTrade;
      } else {
        throw new Error("No tick arrays");
      }
  ...
  const loadPrices = async () => {
    ...
    try {
      ...
      const spotPrice0_in_1 = calculateSpotPrice(
        ctx.poolStateAcct,
        true,
        ctx.mint0.decimals,
        ctx.mint1.decimals,
  ...
  async function handleSwap() {
    ...
    try {
      ...
      const [tickArrayLeft] = getTickArrayAddress(
        ctx.poolAddr,
        startTickIndex - tickArraySizeTimesSpacing,
        pgmId,
      );
      const [tickArrayRight] = getTickArrayAddress(
        ctx.poolAddr,
        startTickIndex + tickArraySizeTimesSpacing,
        pgmId,
      );
      ...
      if (lastEditedField === "input") {
        ...
        const minimumAmountOut = quote.receiveAmount
        ...
        try {
          ...
          await connection
            .confirmTransaction(tx, "confirmed")
            .catch(() => null);
          void refreshPools();
          await loadPrices();
          setStatus(null);
          ...
          if (wallet.publicKey) {
            ...
            addTransaction(tx, `Swapped ${getShortTokenName(direction.inputMint.toBase58())} for ${getShortTokenName(direction.outputMint.toBase58())}`, 'Swap', true);
            refreshPositions();
          }
          return;
        } catch (err: any) {
        ...
        const slippageBps = Math.floor(slippage * 100);
        const maximumInputPreFee = quote.maxInputPreFee
          .mul(new BN(10000 + slippageBps))
          .div(new BN(10000));
  ...
  async function showSendErrorDetails(err: any, hintAddress?: PublicKey) {
    try {
      if (isAlreadyProcessedError(err)) {
        if (hintAddress) {
          try {
            const sigs = await callWithRetry(() => connection.getSignaturesForAddress(hintAddress, {
              limit: 1
            }));
      ...
      try {
        ...
        const sig =
          ...
          (typeof err.message === "string" &&
            err.message.match(/[A-Za-z0-9]{60,88}/)?.[0]) ||
          null;
        if (sig) {
          const tx = await (connection as any)
          ...
          if (txLogs && txLogs.length) {
            ...
            setStatus(
              'Transaction processed. Click "Details" to view RPC logs.',
            );
            return;
          }
        }
  ...
  const inputBalance = isToken0ToToken1
    ? userBalances?.token0
    : userBalances?.token1;
  const outputBalance = isToken0ToToken1
    ? userBalances?.token1
    : userBalances?.token0;
  const cleanAmountIn = String(amountIn || "").replace(/,/g, "");
  const cleanAmountOut = String(amountOut || "").replace(/,/g, "");
  const isInputBalanceExceeded =
    inputBalance != null &&
    Number(cleanAmountIn || "0") > parseBalanceValue(inputBalance);
  const isOutputBalanceExceeded =
    outputBalance != null &&
    Number(cleanAmountOut || "0") > parseBalanceValue(outputBalance);
  const activeBalanceExceeded =
    isInputBalanceExceeded || isOutputBalanceExceeded;
  const hasValidSwapAmount =
    Number(cleanAmountIn || "0") > 0 || Number(cleanAmountOut || "0") > 0;
  ...
  const firstTokenSymbol = showInversePrice
    ? `${priceToken1}`
    : `${priceToken0}`;
  const secondTokenSymbol = showInversePrice
    ? `${priceToken0}`
    : `${priceToken1}`;
  const priceValue = showInversePrice
    ? priceDetails?.token1ToToken0
    : priceDetails?.token0ToToken1;
              ...
              <div className="swap-card">
                  ...
                  <div className="swap-title">
                    ...
                    <div className="swap-pool-name-container">
                      ...
                      >
                        ...
                        {poolHoverInfo && (
                          <div
                            className="swap-pool-hover-card"
                            onMouseEnter={showPoolHover}
                            onMouseLeave={hidePoolHover}
                          >
                            <div className="swap-hover-row">
                              ...
                              <button
                                ...
                                onClick={() => copyText(poolHoverInfo.poolId)}
                                title="Copy pool id"
                            ...
                            <div className="swap-hover-row">
                              ...
                              <button
                                className="swap-copy-btn"
                                onClick={() => copyText(poolHoverInfo.token0)}
                                title="Copy token0"
                            ...
                            <div className="swap-hover-row">
                              ...
                              <button
                                className="swap-copy-btn"
                                onClick={() => copyText(poolHoverInfo.token1)}
                                title="Copy token1"
                ...
                <div className="swap-pool-selector-wrapper">
                  ...
                  <div className="swap-pool-selector-container">
                    ...
                    >
                      <span className="swap-pool-selected">
                        {getPoolSelectorLabel()}
                      </span>
                      <span className="swap-pool-selector-arrow">
                        {showPoolSelector ? "▲" : "▼"}
                      </span>
                    </button>
                      ...
                      <div className="swap-pool-dropdown">
                        <div className="swap-pool-search-container">
                          ...
                          {searchQuery && (
                            <button
                              type="button"
                              className="swap-pool-search-clear"
                        ...
                        {loadingPools ? (
                          ...
                          filteredPools.map((pool) => {
                            ...
                            const poolName = `${name0}-${name1}`;
                            const feeTier = getFeeFromTradeFeeRate(
                              pool.tradeFeeRate,
                            );
                            return (
                              <button
                ...
                <div className="swap-body">
                  <div className="swap-panel pool-details-panel">
                    ...
                    {activePool ? (
                      <div className="pool-details-content">
                        ...
                        <div className="pool-details-grid">
                          <div className="pool-details-card">
                            <span className="pool-details-card__label">
                              Current Price
                            </span>
                            <span className="pool-details-card__value">
                              {priceText}
                            </span>
                          </div>
                          ...
                          <div className="pool-details-card">
                            ...
                            <div className="pool-details-balances">
                              <div className="pool-balance-row">
                                <span className="pool-balance-token">
                                  {getShortTokenName(token0Str)}{" "}
                                </span>
                                <span className="pool-balance-amount">
                                  {userBalances?.token0 != null
                                    ? formatAmount(userBalances.token0)
                                    : "0"}
                                </span>
                              </div>
                              <div className="pool-balance-row">
                                <span className="pool-balance-token">
                                  {getShortTokenName(token1Str)}{" "}
                                </span>
                                <span className="pool-balance-amount">
                                  {userBalances?.token1 != null
                                    ? formatAmount(userBalances.token1)
                                    : "0"}
                                </span>
                              </div>
                            </div>
                      ...
                      <div className="pool-details-empty">
                  ...
                  <div className="swap-panel">
                    <div className="swap-amount-header">
                      ...
                      <div className="swap-slippage-container">
                        <button
                          ...
                          onClick={() =>
                            setShowSlippageSelector(!showSlippageSelector)
                          }
                        ...
                        {showSlippageSelector && (
                          <div className="swap-slippage-overlay">
                            <div className="swap-slippage-overlay-title">
                              Max Slippage
                            </div>
                    ...
                    <form className="swap-form" onSubmit={onSubmit}>
                      <div
                        className={`swap-input-card ${isInputBalanceExceeded ? "swap-input-card--invalid" : ""}`}
                      >
                        <div className="swap-input-card-header">
                          ...
                          <div className="swap-input-card-balance-wrap">
                            <img
                              src={walletIcon}
                              alt="Wallet"
                              className="wallet-mini-icon"
                            />
                            <span className="swap-input-card-balance-val">
                              {inputBalance != null
                        ...
                        <div className="swap-input-card-row">
                          <div
                            className="swap-token-select-pill"
                            onClick={() => {
                          ...
                          >
                            <div
                              className="swap-token-logo-sphere"
                      ...
                      <div className="swap-direction-toggle">
                        ...
                        >
                          <img
                            ...
                            alt="arrow"
                            className="swap-direction-toggle__arrow"
                          />
                          <img
                            src={swapIcon}
                            alt="swap"
                            className="swap-direction-toggle__swap"
                          />
                        </button>
                      </div>

                      <div
                        className={`swap-input-card ${isOutputBalanceExceeded ? "swap-input-card--invalid" : ""}`}
                      >
                        <div className="swap-input-card-header">
                        ...
                        <div className="swap-price-strip">
                          ...
                          <div className="swap-price-strip__toggle">
                            <button
                              ...
                              onClick={() =>
                                setShowInversePrice((current) => !current)
                              }
                              aria-label={
                                showInversePrice
                                  ? "Show token0 to token1 price"
                                  : "Show token1 to token0 price"
                              }
                            >
                              <img
                                src={swapIcon}
                                alt="toggle price"
                                className="swap-price-strip__icon"
                              />
                            </button>
                          </div>
                        </div>
                      ...
                      {hasValidSwapAmount && (
                        <div
                          style={{
                            marginBottom: "16px",
                            padding: "16px",
                            background: "var(--if-background-surface)",
                            borderRadius: "12px",
                            fontSize: "13px",
                            color: "var(--if-text-secondary)",
                            display: "flex",
                        ...
                        >
                          ...
                          {lastEditedField === "input" ? (
                            ...
                            >
                              ...
                              <span>
                                {minReceived.toFixed(6)} {outputTokenShort}
                              </span>
                            </div>
                          ) : (
                            <div
                            ...
                            >
                              ...
                              <span>
                                {maxInput.toFixed(6)} {inputTokenShort}
                              </span>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="swap-actions-row">
                        ...
                        >
                          ...
                          ) : activeBalanceExceeded ? (
                            `Insufficient ${inputTokenShort} Balance`
                          ) : !hasValidSwapAmount ? (
                            "Enter an amount"
                          ) : (
                            "Swap"

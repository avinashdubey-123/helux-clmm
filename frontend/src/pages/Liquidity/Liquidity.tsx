...
function PoolRow({ pool, navigate }: { pool: PoolRowData; navigate: any }) {
  ...
  if (pool.isActiveLiquidityLoading) {
    liquidityDisplay = <div className="inline-loader"><div></div><div></div><div></div><div></div></div>
  ...
  return (
    <tr key={pool.poolPda} className="lp-row">
        ...
        <div className="lp-td-pool-inner">
          <div className="lp-hover-wrapper" onMouseEnter={() => { void handleIconHover() }} onMouseLeave={handleIconLeave} style={{ display: 'inline-block' }}>
            <div className="lp-pool-icons" style={{ cursor: 'pointer' }} title="Hover to view pool info">
              <span className="lp-icon lp-icon-a" style={{ background: token0Color, zIndex: 1 }}>{token0Name.slice(0, 4)}</span>
              <span className="lp-icon lp-icon-b" style={{ background: token1Color }}>{token1Name.slice(0, 4)}</span>
            </div>
            {hoverInfo && (
      ...
      <td className="lp-td lp-col-fee">{((pool.tradeFeeRate || 0) / 10000).toFixed(2)}%</td>
        ...
        <div className="lp-row-actions">
          <div className="lp-swap-tooltip-wrapper">
            <button className="lp-swap-btn" onClick={onSwap} title="Swap">
              <img src="/src/assets/swap.svg" alt="Swap" style={{ width: '16px', height: '16px' }} />
            </button>
...
const Liquidity = () => {
  ...
  const poolCount = useMemo(() => pools.length, [pools])

  const filteredPools = useMemo(() => {
    if (!searchQuery) return pools
    const q = searchQuery.toLowerCase()
    return pools.filter(p => {
      const name = `${p.tokenMint0.slice(0, 4)}-${p.tokenMint1.slice(0, 4)}`.toLowerCase()
      return (
      ...
      <div className="lp-top">
        ...
        <div className="lp-stats">
          <div className="lp-stat-card">
            ...
            <span className="lp-stat-value">{poolCount} Pools</span>
          </div>
        </div>
      </div>
